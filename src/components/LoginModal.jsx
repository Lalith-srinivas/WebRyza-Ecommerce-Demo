  ...
  return (
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="login-modal">
        ...
        <h2 className="modal-title">India's last minute app</h2>
        <p className="modal-subtitle">
          {tab === "login" ? "Log in or " : ""}
          <button className="tab-link" onClick={() => { setTab(tab === "login" ? "signup" : "login"); setError(""); }}>
            {tab === "login" ? "Sign up" : "← Back to Login"}
          </button>
        </p>
        ...
        <form onSubmit={handleSubmit} className="login-form">
          ...
          <button type="submit" className="btn-primary btn-full" disabled={loading}>
            {loading ? <span className="spinner" /> : tab === "login" ? "Continue" : "Create Account"}
          </button>
        </form>
